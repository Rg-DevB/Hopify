...
import { 
  ...
  Stethoscope, Bed, Pill, TrendingUp,Truck, FlaskConical, Bell, Video, BrainCircuit, Monitor, MessageCircle, Siren, ShieldCheck, Scissors
} from "lucide-react";
...
export default function Sidebar() {
  ...
  const NavContent = () => (
    <>
      {/* Logo */}
      <Link href="/" className={cn("p-6 flex items-center gap-3 hover:opacity-90 transition-all", collapsed ? "justify-center px-2" : "px-8")}>
        <div className="w-10 h-10 rounded-[14px] bg-gradient-to-br from-teal-500 to-cyan-500 flex items-center justify-center shadow-xl shadow-teal-500/20 shrink-0 floating">
          <HeartPulse className="w-6 h-6 text-white" />
        </div>
        {!collapsed && (
          <div className="flex flex-col">
            <span className="text-xl font-black text-slate-900 dark:text-white tracking-tighter leading-none">Hopify</span>
            <span className="text-[10px] font-black text-teal-600 uppercase tracking-[0.2em] mt-1">Medical ERP</span>
          </div>
        )}
      </Link>
